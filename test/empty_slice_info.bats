...
# is peachy.
#
@test "Test where input slice.info file is empty" {

  # verify $KEPLER_SH is in path if not skip this test
  skipIfKeplerNotInPath
  mkdir -p "$THE_TMP/foo/data"
  echo "hi" > "$THE_TMP/foo/data/slice.png"
  echo "" > "$THE_TMP/foo/slice.info"
  # Run kepler.sh
  ...
  [ "${lines[1]}" == "detailed.error.message=width (x) of image not found in $THE_TMP/foo/slice.info" ]
