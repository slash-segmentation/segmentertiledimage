...
#
@test "Test where convert command does not exist" {
  ...
  # Run kepler.sh
  run $KEPLER_SH -runwf -redirectgui $THE_TMP -CWS_jobname jname -CWS_user joe -CWS_jobid 123 -inputimagedir "$THE_TMP/foo" -convertCmd "$THE_TMP/noexist" -CWS_outputdir $THE_TMP $WF
  ...
  [[ "${lines[1]}" == "detailed.error.message=Caught exception trying to run $THE_TMP/noexist $THE_TMP/foo/data/slice.png -crop 128x128 -set filename:tile \"r%[fx:page.y/128]_c%[fx:page.x/128]\" +repage +adjoin $THE_TMP/data\"/0-%[filename:tile].png\" : Cannot run program"* ]]
