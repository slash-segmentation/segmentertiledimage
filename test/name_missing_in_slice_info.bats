...
# is peachy.
#
@test "Test where original.name missing from slice.info" {

  # verify $KEPLER_SH is in path if not skip this test
  skipIfKeplerNotInPath
  mkdir -p "$THE_TMP/foo/data"
  echo "hi" > "$THE_TMP/foo/data/slice.png"
  echo -e "x=2\ny=2" > "$THE_TMP/foo/slice.info"
  # Run kepler.sh
  ...
  [ "${lines[1]}" == "detailed.error.message=name (original.name) of image not found in $THE_TMP/foo/slice.info" ]
